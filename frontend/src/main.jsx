    ...
    <>
      <header className="topbar">
        ...
        <nav>
          ...
          {user ? <NavLink to="/settings">设置</NavLink> : <NavLink to="/login">登录</NavLink>}
        </nav>
        {user && (
          <div className="accountPill" title={user.bio || user.username}>
            {user.avatar_url ? <img src={user.avatar_url} alt="头像" /> : <span>{(user.display_name || user.username || 'U').slice(0, 1).toUpperCase()}</span>}
            <strong>{user.display_name || user.username}</strong>
          </div>
        )}
        {user && <button className="iconButton" title="退出登录" onClick={logout}><LogOut size={18} /></button>}
      ...
      {(user || authChecked) && <Link className="composeFab" title="新建文章" to="/edit/new"><PenLine size={20} /></Link>}
    </>
  );
}
